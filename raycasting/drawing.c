...
/*   Updated: 2023/11/30 22:29:41 by omakran          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	drawing_ciel_and_floor(t_game *game)
{
	...
	while (i < HEIGHT)
	{
		...
		{
			if (i < (HEIGHT / 2))
				mlx_put_pixel(game->mini_map, j, i, ft_pixel(0, 255, 255, 0));
			else
				mlx_put_pixel(game->mini_map, j, i, ft_pixel(0, 300, 355, 255));
			j++;
		}
		i++;
	}
...
void	draw_it(t_game *game, float x_tx , mlx_texture_t *texture, int id)
{
	...
	start_tmp_y = ((HEIGHT / 2) - (game->player_pos->wall_height));
