...
/*   Updated: 2023/12/17 13:09:58 by yoamzil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if ((game->player_pos->is_facing_up
			...
			|| (game->player_pos->is_facing_up
				&& check_if_hit_wall(game, game->hori->x_hori,
					game->hori->y_hori)))
			break ;
		game->hori->x_hori += game
			->player_pos->sign * SQUAR_SIZE / tan(*ray_angle);
...
void	horizontal_steps(t_game *game, float ray_angle)
{
	...
	game->hori->x_hori = game->player_pos->x
		+ (game->hori->y_hori - game->player_pos->y) / tan(ray_angle);
